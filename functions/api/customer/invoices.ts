...
export const onRequestGet: PagesFunction<Env> = async (context) => {
    ...
    try {
      ...
      WHERE (
            ...
            OR EXISTS (
                SELECT 1
                FROM projects p2
                LEFT JOIN quotes q2 ON q2.id = p2.quote_id
                WHERE p2.id = i.project_id
                  AND (
    ...
    `;
        ...
        const invoiceResults = await env.DB.prepare(listQuery)
            .bind(customerId, sessionEmail, sessionEmail, sessionEmail, statusFilter, statusFilter, limit, offset)
            .all<InvoiceRow>();
    ...
    `
            ...
            .bind(customerId, sessionEmail, sessionEmail, sessionEmail, statusFilter, statusFilter)
            .first<{ total: number }>();
      ...
      WHERE (
            ...
            OR EXISTS (
                ...
                FROM projects p2
                LEFT JOIN quotes q2 ON q2.id = p2.quote_id
                WHERE p2.id = invoices.project_id
                  AND (
                    LOWER(COALESCE(q2.contact_email, '')) = ?
                    OR LOWER(COALESCE(q2.customer_email, '')) = ?
                  )
    ...
    `
            ...
            .bind(customerId, sessionEmail, sessionEmail, sessionEmail)
